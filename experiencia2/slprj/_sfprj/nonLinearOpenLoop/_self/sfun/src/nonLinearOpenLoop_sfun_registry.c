...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_nonLinearOpenLoop_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_nonLinearOpenLoop_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_nonLinearOpenLoop_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "nonLinearOpenLoop") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_nonLinearOpenLoop_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_nonLinearOpenLoop_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "nonLinearOpenLoop") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_nonLinearOpenLoop_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_nonLinearOpenLoop_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"nonLinearOpenLoop")) {
    return(sf_nonLinearOpenLoop_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void nonLinearOpenLoop_terminator(void);
...
extern void nonLinearOpenLoop_initializer(void);
extern void nonLinearOpenLoop_register_exported_symbols(SimStruct* S);
extern void nonLinearOpenLoop_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  nonLinearOpenLoop_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      nonLinearOpenLoop_debug_initialize();
    }

    nonLinearOpenLoop_initializer();
  }
